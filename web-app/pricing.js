...
// Minimum transaction value to avoid zero-value tx in MetaMask
const MIN_TX_PRICE_FLR = 0.01;
...
function calculateBasePrice(dataType, polygon, aiAnalysisEnabled, startDate, endDate) {
    ...
    // Enforce minimum to ensure a non-zero payable amount
    if (finalBasePrice < MIN_TX_PRICE_FLR) {
        finalBasePrice = MIN_TX_PRICE_FLR;
    }
...
async function applyRandomVariance(basePrice, params) {
    try {
        ...
        const adjustedFinal = Math.max(MIN_TX_PRICE_FLR, result.finalPrice);
        
        return {
            finalPrice: adjustedFinal,
            randomFactor: result.variationFactor,
            vrfResult: { ...result, finalPrice: adjustedFinal },
        ...
        const clamped = Math.max(MIN_TX_PRICE_FLR, finalPrice);
        return {
            finalPrice: Math.round(clamped * 100) / 100,
